...
def get_save_path():  # data目录的绝对路径
    root_dir = get_parent_dir()
    return os.path.join(root_dir, "saved_models")    
...
class Config:
    ...
    #model_name =  get_code() + model_postfix[used_frame]

    # 路径参数
    train_data_path = get_data_path()+"/py"
...
class Data:
    ...
    def read_data(self):                # 读取初始数据
        if self.config.debug_mode:
            init_data = pd.read_csv(self.config.train_data_path+self.code+'.csv', nrows=self.config.debug_num,
                                    usecols=self.config.feature_columns)
        else:
            init_data = pd.read_csv(self.config.train_data_path+self.code+'.csv', usecols=self.config.feature_columns)
...
def main(code,good,again):
    ...
    try:
        ...
        if config.do_predict:
            ...
            pred_result = predict(config, test_X,code)       # 这里输出的是未还原的归一化预测数据
            
            label_data = data_gainer.data[data_gainer.train_num + data_gainer.start_num_in_test : ,
                                            config.label_in_feature_index]
            ...
            loss = np.mean((label_data[config.predict_day:] - predict_data[:-config.predict_day] ) ** 2, axis=0)
            loss_norm = loss/(data_gainer.std[config.label_in_feature_index] ** 2)
            logger.info("The mean squared error of stock {} is ".format(label_name) + str(loss_norm))
            
            
            label_X = range(data_gainer.data_num - data_gainer.train_num - data_gainer.start_num_in_test)
            predict_X = [ x + config.predict_day for x in label_X]
            ...
            for i in range(label_column_num):
                ...
                #plt.plot(predict_X, predict_data[:, i], label='predict')
                #plt.title("Predict stock {} price with {}".format(label_name[i], config.used_frame))
                info.append(str(np.squeeze(predict_data[-config.predict_day:, i])))
